...
export default function Analysis() {
    ...
    const fetchAnalysisData = async () => {
      ...
      try {
        ...
        if (summaryResponse.status === 200 && trendsResponse.status === 200) {
          ...
          setError(
            summaryResponse.message || trendsResponse.message || "An error occurred while fetching the data."
          );
        }
      } catch (err: any) {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-rose-50 to-white flex flex-col">
      ...
      >
        <h1 className="text-3xl font-bold text-rose-800 mb-6 text-center">Your Mood Analysis</h1>
          ...
          <>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
              <div className="h-[400px]">
                <h2 className="text-xl font-semibold text-gray-700 mb-4">Daily Mood Summary</h2>
            ...
            <div className="mt-6">
              ...
              <p className="text-gray-800">{generateInsights()}</p>
            </div>
          </>
        )}
      </motion.section>
